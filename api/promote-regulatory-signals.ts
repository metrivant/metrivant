...
async function handler(req: ApiReq, res: ApiRes) {
  ...
  Sentry.captureCheckIn({ monitorSlug: "promote-regulatory-signals", status: "in_progress" });
  ...
  try {
    ...
    if (rowsClaimed === 0) {
      Sentry.captureCheckIn({ monitorSlug: "promote-regulatory-signals", status: "ok" });
    ...
    Sentry.captureCheckIn({ monitorSlug: "promote-regulatory-signals", status: "error" });
    await Sentry.flush(2000);
    throw error;
  }
}
