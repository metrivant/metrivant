    ...
    for (const event of events) {
      ...
      try {
        ...
        const dedup = await findCrossPoolDuplicate(
          event.competitor_id, meta.investorEventType, currentExcerpt, event.published_at ?? new Date().toISOString()
        );
        if (dedup.isDuplicate) {
          await supabase.from("pool_events").update({ normalization_status: "duplicate" }).eq("id", event.id);
          rowsDuplicate += 1;
          void recordEvent({ run_id: runId, stage: "investor_promote", status: "skipped", duration_ms: elapsed(), metadata: { pool_event_id: event.id, reason: `cross_pool_dedup:${dedup.matchReason}`, matched_signal: dedup.matchedSignalId } });
          continue;
        }
