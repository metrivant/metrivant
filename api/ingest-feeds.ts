...
async function handler(req: ApiReq, res: ApiRes) {
  ...
  const checkInId = Sentry.captureCheckIn({ monitorSlug: "ingest-feeds", status: "in_progress" });
  ...
  try {
    ...
    Sentry.captureCheckIn({ monitorSlug: "ingest-feeds", status: "error", checkInId });
    await Sentry.flush(2000);
    throw error;
  }
}
