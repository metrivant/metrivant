...
// Maximum feed entries per ingestion run.
...
async function handler(req: ApiReq, res: ApiRes) {
  ...
  const checkInId = Sentry.captureCheckIn({ monitorSlug: "ingest-investor-feeds", status: "in_progress" });
  ...
  try {
      ...
      try {
        ...
        for (const entry of freshEntries) {
          ...
          newRows.push({
            ...
            event_type:           "investor_update", // classified later by promote-investor-signals
    ...
    Sentry.setContext("run_metrics", {
      stage_name: "ingest-investor-feeds",
      feedsTotal,
      feedsIngested,
      feedsFailed,
      eventsInserted,
    ...
    // Warn when active feeds were fetched successfully but produced zero new events.
    // eventsDuplicate === 0 excludes normal steady-state dedup — this only fires
    // when feeds are empty or all entries exceed MAX_ENTRY_AGE_DAYS.
    if (feedsTotal > 0 && feedsIngested > 0 && eventsInserted === 0 && eventsDuplicate === 0) {
      Sentry.captureMessage("ingest_investor_feeds_empty_entries", "warning");
    }

    Sentry.captureCheckIn({ monitorSlug: "ingest-investor-feeds", status: "ok", checkInId });
    await Sentry.flush(2000);

    res.status(200).json({
      ok: true,
      job: "ingest-investor-feeds",
      feedsTotal,
      feedsIngested,
      feedsFailed,
      eventsInserted,
    ...
    Sentry.captureCheckIn({ monitorSlug: "ingest-investor-feeds", status: "error", checkInId });
    await Sentry.flush(2000);
    throw error;
  }
}

export default withSentry("ingest-investor-feeds", handler);
