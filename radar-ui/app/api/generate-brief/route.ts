...
function buildBriefMarkdown(
  ...
  // Market Context section — assembled from sector_narratives.
  // Uses GPT-generated narrative_summary when available, falls back to metadata.
  if (narratives.length > 0) {
    lines.push(`## Market Context`, ``);
    for (const n of narratives) {
      const date = n.last_detected_at.slice(0, 10);
      const confPct = Math.round(n.confidence_score * 100);
      if (n.narrative_summary) {
        lines.push(`- **${n.theme_label}** — ${n.narrative_summary} _(${n.article_count} articles, ${n.source_count} sources, ${confPct}% confidence)_`);
      } else {
        lines.push(
      ...
      if (org.sector) {
        try {
          ...
          const { data: narRows } = await sb
            ...
            .select("theme_label, article_count, source_count, confidence_score, last_detected_at, narrative_summary")
