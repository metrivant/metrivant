...
function TriggerButton({ onClick, isOpen }: { onClick: () => void; isOpen: boolean }) {
  ...
  return (
    <button
      ...
      style={{
        borderColor: isOpen ? "rgba(0,180,255,0.35)" : "#0e1e0e",
        background: isOpen
          ? "linear-gradient(135deg, rgba(0,180,255,0.08) 0%, rgba(2,8,2,0.95) 60%)"
          : "rgba(2,8,2,0.80)",
        boxShadow: isOpen
          ? "0 0 20px rgba(0,180,255,0.12), inset 0 0 15px rgba(0,180,255,0.04)"
          : hovered
            ? "0 0 12px rgba(0,180,255,0.06)"
            : "none",
      }}
    >
      {/* 3D valve symbol */}
      ...
      <div className="flex flex-col text-left">
        <span
          ...
          style={{ color: isOpen ? "#00B4FF" : "rgba(148,163,184,0.6)" }}
        >
